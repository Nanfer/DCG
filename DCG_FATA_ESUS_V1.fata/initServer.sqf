...
_SEN_blacklistLocation = ["Sirika"]; // locations in array will be removed from DCG. You can add to array, but do not remove current locations from list
...
if !(isNil "_grp") then {
	if !((units _grp) isEqualTo []) then {
		[units _grp,{
			{
				_action = [format ["SEN_doctor%1",_forEachIndex],"Operar paciente","",{
					[
						10,
						[],
						{
							[objNull, player] call ace_medical_fnc_treatmentAdvanced_fullHealLocal;
							hintSilent "Ya estas bien. Ten mas cuidado la proxima vez.";
						},
						{
